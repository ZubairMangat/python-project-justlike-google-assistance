...
# Initialize the recognizer and text-to-speech engine for English
recognizer = sr.Recognizer()
engine = pyttsx3.init()

# Global variable to store the current language (English by default)
current_language = 'en'

# Function to play audio for Urdu using gTTS
def speak_urdu(text):
...
# Function to play audio for English using pyttsx3
def speak(text):
    if current_language == 'en':
        engine.say(text)
        engine.runAndWait()
    elif current_language == 'ur':
        speak_urdu(text)
...
def process_command(command):
    ...
    if "switch to urdu" in command:
        global current_language
        current_language = 'ur'
        speak("Urdu mein kaam shuru.")
    ...
    while True:
        try:
            with sr.Microphone() as source:
                ...
                # Capture audio and recognize in current language
                audio = recognizer.listen(source, timeout=7, phrase_time_limit=5)

                # Recognize speech based on the current language
                if current_language == 'en':
                    word = recognizer.recognize_google(audio, language='en-US')
                elif current_language == 'ur':
                    word = recognizer.recognize_google(audio, language='ur')
                ...
                if word.lower() == "abdullah":
                    ...
                    # Listen for the command in the current language
                    with sr.Microphone() as source:
                        print("Listening for command...")
                        recognizer.adjust_for_ambient_noise(source, duration=0.5)
                        audio = recognizer.listen(source, timeout=7, phrase_time_limit=5)

                        # Recognize command in current language
                        if current_language == 'en':
                            command = recognizer.recognize_google(audio, language='en-US')
                        elif current_language == 'ur':
                            command = recognizer.recognize_google(audio, language='ur')
